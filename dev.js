...
class DeveloperEnvironment {
    ...
    async buildDocker() {
        try {
            await run('docker network create dev-bingo-proxy');
        } catch (err) {
            //
        }
    ...
    async addCertificates() {
        ...
        try {
            ...
            await run('cd ./certs && mv ssl.crt dev-bingo.local.crt && mv ssl.key dev-bingo.local.key');
...
const validCommands = ['setup', 'start', 'restart', 'stop'];
